...
class AutoResearchSkill {
    ...
    async run(queryFn) {
        ...
        const experiment = this.tracker.createExperiment(this.config.task, this.config.datasetPath, 1, this.config.experimentName);
        ...
        for (let iteration = 1; iteration <= this.config.maxIterations; iteration++) {
            ...
            const currentExperiment = this.tracker.createExperiment(this.config.task, this.config.datasetPath, iteration, this.config.experimentName);
    ...
    async getSpecification(queryFn, experiment) {
        const understandingConfig = agents_1.agentConfigs.understanding;
...
Save the final specification as specification.md in the experiment directory: ${experiment.baseDir}/specification.md`;
        ...
        if (inputMatch)
            spec.inputFormat = inputMatch[1].trim();
        if (outputMatch)
            spec.outputFormat = outputMatch[1].trim();
        if (objectiveMatch)
            spec.trainingObjective = objectiveMatch[1].trim();
        if (requirementsMatch)
            spec.codingRequirements = requirementsMatch[1].trim();
        if (directionMatch)
            spec.researchDirection = directionMatch[1].trim();
        if (patternsMatch)
            spec.extractedPatterns = patternsMatch[1].trim();
        return spec;
    }
    async researchEvaluateLoop(queryFn, experiment) {
        const researcherConfig = agents_1.agentConfigs.researcher;
        const evaluatorConfig = agents_1.agentConfigs.evaluator;
        let currentPlan = '';
        let feedback = '';
        while (true) {
            ...
            if (evaluation.outcome === types_1.RefinementOutcome.APPROVED) {
    ...
    buildResearchPrompt(experiment, feedback) {
...
---`;
    ...
    parseEvaluation(output) {
        ...
        const outcome = outcomeMatch?.[1]?.toLowerCase() === 'approved'
            ? types_1.RefinementOutcome.APPROVED
            : types_1.RefinementOutcome.REJECTED;
        const feedback = feedbackMatch?.[1]?.trim() || 'No feedback provided';
        return { outcome, feedback };
    }
    parsePlan(researchOutput) {
    ...
    async writeCode(queryFn, experiment, plan) {
        const coderConfig = agents_1.agentConfigs.coder;
...
Ensure all imports are correct and the code is runnable. Follow the extracted coding conventions from above.`;
    ...
    async recordAndAnalyze(queryFn, experiment, result) {
        const recorderConfig = agents_1.agentConfigs.recorder;
...
Be specific about what to change. Save this summary to the experiment directory.`;
    ...
    formatIterationResult(experiment, result) {
